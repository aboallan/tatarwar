...
if ($action === 'logout') {
    ...
    redirect_with_flash('success', 'Signed out', 'You have been signed out safely.', 'login.php');
}
...
try {
    ...
    $target = $action === 'register' ? 'register.php' : ($action === 'update' ? 'dashboard-updates.php#attendance-update' : 'login.php');
...
if ($action === 'login') {
    ...
    redirect_with_flash('success', 'Welcome back', $message, 'dashboard-agenda.php');
}
...
if ($action === 'update') {
    ...
    if ($uniqueCode === '') {
        redirect_with_flash('error', 'Accreditation code required', 'Provide your approved accreditation code to manage attendance.', 'dashboard-updates.php#attendance-update');
    }

    if (!in_array($attendance, ['pending', 'attend', 'decline'], true)) {
        redirect_with_flash('error', 'Invalid status', 'Choose a valid attendance status from the list.', 'dashboard-updates.php#attendance-update');
    }

    if ($location !== '' && !in_array($location, $locations, true)) {
        redirect_with_flash('error', 'Unknown location', 'Select a meeting venue from the approved list.', 'dashboard-updates.php#attendance-update');
    }

    if (!in_array($scope, ['all', 'specific'], true)) {
        redirect_with_flash('error', 'Invalid scope', 'Specify whether participation includes all departments or a specific one.', 'dashboard-updates.php#attendance-update');
    }

    if ($scope === 'specific') {
        if (!in_array($focus, $departments, true)) {
        redirect_with_flash('error', 'Department not found', 'Choose the target department from the list.', 'dashboard-updates.php#attendance-update');
        }
    } else {
        $focus = '';
    }
    ...
    if ($stmt->rowCount() === 0) {
        redirect_with_flash('error', 'Account not found', 'No record matched the accreditation code provided.', 'dashboard-updates.php#attendance-update');
    }
    ...
    redirect_with_flash('success', 'Attendance updated', 'Your preferences and meeting summary were saved successfully.', 'dashboard-updates.php');
}
