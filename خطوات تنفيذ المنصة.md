...
2. The dashboard now lives across dedicated pages (`dashboard-agenda.php`, `dashboard-updates.php`, `dashboard-summary.php`, `dashboard-venues.php`, `dashboard-attendance.php`). Each shares the same sidebar and header shell, so clicking a link loads a focused workspace instead of stacking every section in one page.
